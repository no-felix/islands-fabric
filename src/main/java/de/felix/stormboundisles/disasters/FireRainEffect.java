...
/**
 * Example effect: Drops fireballs randomly in the zone.
 */
public class FireRainEffect implements DisasterEffect {
	private final int numFireballs;

	public FireRainEffect(int numFireballs) {
	...
	public void apply(ServerWorld world, IslandZone zone, List<ServerPlayerEntity> players) {
		...
		for (int i = 0; i < numFireballs; i++) {
			// Randomly pick X/Z in bounding box, then check isInside
			...
			while (tries < 10) {
				...
				if (zone.isInside(pos)) {
					// Spawn fireball or fire at this position
					// TODO: Actual fireball entity spawn
					world.setBlockState(pos.down(), net.minecraft.block.Blocks.FIRE.getDefaultState());
					break;
				}
				tries++;
			}
